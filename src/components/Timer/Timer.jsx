...
const Timer = () => {
  ...
  return (
    <div>
      {showTimer ? (
        <div className="flex items-center space-x-2 bg-dark-fill-3 rounded cursor-pointer p-1.5 hover:bg-dark-fill-2">
          <div>{formatTime(time)}</div>
          <FiRefreshCcw onClick={()=>{handleClockClick(); setTime(0)}}/>
        </div>
      ) : (
